...
export function FeaturedRoomCard({ room, onOpen }) {
  return (
    <article className="card flex flex-col cursor-pointer transition-transform hover:-translate-y-1" onClick={() => onOpen(room)}>
      <Img label={room.name + " · photo"} style={{ height: 248 }} />
      <div style={{ padding: "22px 24px 24px", display: "flex", flexDirection: "column", flex: 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
          <span className="badge badge--soft"><span className="dot" style={{ color: "var(--color-gold)" }} />{room.type}</span>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginTop: "auto" }}>
          ...
          <button className="btn btn--navy" onClick={(e) => { e.stopPropagation(); onOpen(room); }} style={{ padding: "12px 22px" }}>
            Book Now
          </button>
        </div>
      </div>
